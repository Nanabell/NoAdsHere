        ...
        private static async Task ExecutePenalty(ICommandContext context, Violator violator)
        {
            ...
            foreach (var penalty in penalties.OrderBy(p => p.RequiredPoints))
            {
                ...
                switch (penalty.PenaltyType)
                {
                    ...
                    case PenaltyTypes.InfoMessage:
                    {
                        Logger.Info(
                            $"User {context.User} exceeded the limit for Penalty {penalty.PenaltyId}({penalty.RequiredPoints}) on {context.Guild}. Executing Penalty on Level InfoMessage");
                        await InfoMessagePenalty.SendAsync(context, penalty.Message);
                        break;
                    }
                    case PenaltyTypes.WarnMessage:
                    {
                        Logger.Info(
                            $"User {context.User} exceeded the limit for Penalty {penalty.PenaltyId}({penalty.RequiredPoints}) on {context.Guild}. Executing Penalty on Level WarnMessage");
                        await WarnMessagePenalty.SendAsync(context, penalty.Message);
                        break;
                    }
                    case PenaltyTypes.Kick:
                    {
                        Logger.Info(
                        ...
                        await KickPenalty.KickAsync(context, penalty.Message);
                        break;
                    }
                    ...
                    {
                        Logger.Info(
                            $"User {context.User} exceeded the limit for Penalty {penalty.PenaltyId}({penalty.RequiredPoints}) on {context.Guild}. Executing Penalty on Level Ban");
                        await BanPenalty.BanAsync(context, penalty.Message);
                        break;
                    }
                    default:
