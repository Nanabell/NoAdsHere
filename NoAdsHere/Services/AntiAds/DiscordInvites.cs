...
namespace NoAdsHere.Services.AntiAds
{
    ...
    {
        private static readonly Regex Invite = new Regex(@"(?:(?i)discord(?:(?:\.|.?dot.?)(?i)gg|app(?:\.|.?dot.?)com\/invite)\/(?<id>([\w]{10,16}|[a-zA-Z1-9]{4,8})))", RegexOptions.Compiled);
        ...
        private async Task InviteChecker(SocketMessage arg)
        {
            ...
            if (Invite.IsMatch(context.Message.Content))
            {
        ...
        private async Task TryDelete(GuildSetting settings, ICommandContext context)
        {
            ...
            if (guildUser.RoleIds.Any(userRole => settings.Ignorings.Roles.Contains(userRole)))
            {
                return;
            }
